...
namespace Jumper_s_Expedition
{
    public class PauseMenu
    {
        ...
        public PauseMenu(Form1 client, GlobalKeyManager kmg)
        {
            this.game_font = new Font("Impact", 32);

            this.client = client;
            this.keyManager = kmg;
            b_start = new Button(384 / 2 - 32, 216 / 2 - 8, 64, 16, "RESUME", this.client);
        ...
        public void Update()
        {
            if(this.client.paused && this.client.player.health > 0)
            {
                b_start.Update();
                b_difficulty.Update();
                b_quit.Update();
